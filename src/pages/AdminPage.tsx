    ...
    <div style={{ padding: 16 }}>
      <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
        ...
        <button
          ...
          style={{ fontWeight: tab === 'order' ? 'bold' : 'normal' }}
        >
          주문 관리
        </button>
        <button
          onClick={() => setTab('table')}
          style={{ fontWeight: tab === 'table' ? 'bold' : 'normal' }}
        >
          테이블 요약
        </button>
      </div>
      ...
      {tab === 'menu' && (
        <div style={{ marginBottom: 24 }}>
          ...
          <button
            ...
            style={{ background: '#2563eb', color: 'white', padding: '8px 16px', marginLeft: 8 }}
          >
            {editId ? '수정 완료' : '메뉴 추가'}
          </button>
          {editId && (
      ...
      {tab === 'order' && <div>주문 관리 페이지 (목업) - 추후 구현</div>}
      {tab === 'table' && <div>테이블 요약 페이지 (목업) - 추후 구현</div>}
