...
interface OrderDetail { orderId:number; status:string; createdAt:string|null; items:OrderItem[] }
interface TableOrderHistory { tableNumber:number; totalAmount:number; orders:OrderDetail[] }
...
export default function OrderHistoryPage() {
  ...
  useEffect(() => {
    if (!tableId) return setLoading(false)
    ...
    <div className="p-6 max-w-md mx-auto">
      ...
      <ul className="space-y-3">
        {hist.orders.slice(-10).reverse().map(o => (
          <li key={o.orderId} className="border rounded p-3 cursor-pointer"
              onClick={() => setSel(o)}>
      ...
      {sel && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center"
          ...
          <div className="bg-white rounded p-6" onClick={e=>e.stopPropagation()}>
            <h3 className="text-lg mb-2">주문 상세 #{sel.orderId}</h3>
            <p>상태: {sel.status}</p>
            <p>시간: {sel.createdAt && new Date(sel.createdAt).toLocaleString()}</p>
