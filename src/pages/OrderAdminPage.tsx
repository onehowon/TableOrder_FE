...
import {
  getAdminOrders,
  updateAdminOrderStatus      /* ← 이미 api.ts 에 있음 */
} from '../api'
...
export default function OrderAdminPage() {
  ...
  const fetch = ()=> {
    setLoading(true)
    getAdminOrders().then(r=>setOrders(r.data.data)).finally(()=>setLoading(false))
  }
  useEffect(fetch,[])
        ...
        {orders.map(o=>(
          <li key={o.id} className="border border-zinc-700 rounded-xl p-4">
            ...
            <div className="flex gap-2">
              {o.status==='WAITING' && (
                <button onClick={()=>onUpdate(o.id,'COOKING')} className="btn-secondary flex-1">
                  COOKING
                </button>
              )}
              {o.status!=='SERVED' && (
                <button onClick={()=>onUpdate(o.id,'SERVED')}  className="btn-primary flex-1">
                  SERVED
                </button>
              )}
            </div>
