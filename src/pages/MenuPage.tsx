...
export default function MenuPage() {
  ...
  const [qtyMap, setQtyMap] = useState<{ [menuId: number]: number }>({});
  ...
  return (
    <div className="px-2 py-4 max-w-lg mx-auto sm:px-4">
      ...
      <ul className="flex flex-col gap-4">
        {menus.map(menu => (
          <li key={menu.id} className="bg-white rounded-xl shadow p-4 flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="flex-1">
              <p className="font-bold text-lg mb-1">{menu.name}</p>
              <p className="text-gray-500 text-sm mb-1 break-words">{menu.description}</p>
              <p className="font-semibold text-blue-700 mb-2">{menu.price.toLocaleString()}원</p>
              <div className="flex items-center gap-2 mt-2">
                <input
                  type="number"
                  min={1}
                  value={qtyMap[menu.id] || 1}
                  onChange={e => handleQtyChange(menu.id, Number(e.target.value))}
